...
var cats = [
    {
        ...
        playlists: [
            {
                name: "Local Music",
                uri: "spotify:user:jerblack:playlist:2905QeOYepXLhUSlaaGeip"
            }, {
                name: "Shuffle EPs 1",
                uri: "spotify:user:jerblack:playlist:2u1oaWXztlG7Xd5psYiXqL"
            }, {
                name: "Shuffle Playlist 1",
                uri: "spotify:user:jerblack:playlist:5XnrfPufI8J3WuDXSJrj3m"
            }, {
                name: "Shuffle Playlist 2",
                uri: "spotify:user:jerblack:playlist:3L5VxdSBxnUPVhwXCoThiG"
            }, {
                name: "~Shuffle Played",
                uri: "spotify:user:jerblack:playlist:3yMUdAFqso3JSTxXPhqCam"
            }
        ]
    }
]
...
function log(entryType, entryText, logToDashboard) {
    ...
    var spaces = 4; // number of spaces to prepend to entryText lines

    if (logToDashboard == true) {
        if (entryType != "" || entryType != undefined) {
            log.dash('|' + entryType + '| ')
        }
        entryText.forEach(function (t) {
            if (t != "") {
                var entry = "";
                for (i = 0; i < spaces; i++) {
                    entry += "&nbsp";
...
log.dash = function (log_text, padding) {
    var entry = '<div class="log-entry">' + log_text + '</div>';
    var l = $("#log");
    l.append(entry);
    var y = l[0].scrollHeight;
    l.scrollTop(y);
...
dpb = function () {
    var btn = ""
    $("#playlists").append('<li><a href="#" id="pl1">Electronic</a></li>')
}


dashboard.toolButtons = function () {
    $("#skip-back-button").click(play.restart);
    $("#play-button").click(play.toggle);
    $("#next-button").click(play.next);
    $("#archive-track").click(archive.track.current);
    $("#play-shuffle").click(play.shuffle);
    $("#play-starred").click(play.starred);
    $("#remove-artist").click(remove.artist.current);
    $("#remove-album").click(remove.album.current);
    $("#de-dupe").click(dedupe.find);
    $("#delete-queue").click(remove.queue.process);
    $("#add-dupe").click(utils.add.toDupes);
    $("#move-ambient").click(utils.move.toAmbient);
    $("#move-electronic").click(utils.move.toElectronic);
    $("#move-shuffle").click(utils.move.toShuffle);
    $("#move-sahara").click(utils.move.toSahara);

    $("#remove-later-cancel").click(remove.later.cancel);
    $("#remove-later-set").click(remove.later.set);

    $("#star-true").click(star.undo);
    $("#star-false").click(star.current);
    $("#remove-icon").click(remove.track.current);
    $("#refresh-count").click(utils.count);
...
nowPlaying.update.dash = function () {
    var placeholder = "sp://import/img/placeholders/300-album.png";
    var track = player.track;
    if (player.track != null) {

        var t = {
            image: track.image ? track.image : placeholder,
            ...
            uri: track.uri,
            year: (track.album.year ? " (" + track.album.year + ")" : ""),
            starred: track.starred,
            local: track.local
        }

        if (t.local) {
            var img = nowPlaying.getLocalArtPath();
            t.image = img != "" || img != "not_local" ? img : placeholder;
            var u = "url(" + t.image + ")";
            console.log(u);
            $("#np-image").css("background-image", u)
                .attr('href', "#");
            $("#np-title").text(t.title)
                .attr('href', "#");
            $("#np-artist").text(t.artist)
                .attr('href', "#");
            $("#np-album").text(t.album)
                .attr('href', "#");
        } else {
            $("#np-image").css("background-image", "url(" + t.image + ")")
                .attr('href', t.album_uri);
            $("#np-title").text(t.title)
                .attr('href', t.album_uri);
            $("#np-artist").text(t.artist)
                .attr('href', t.artist_uri);
            $("#np-album").text(t.album + t.year)
                .attr('href', t.album_uri);
        }
...
// TODO: local track album art does not display in dashboard, displays fine if track is linked, blank if track is not linked
...
play.playlist = function (spURL) {
    models.Playlist.fromURI(spURL, function (pl) {
        player.play(pl.tracks[0].uri, pl);
    ...
    if (cmd.indexOf('+') != -1) {
        ...
        switch (cmd) {
            ...
            case 'addalbum':
                ...
                break;
            case 'moveelectronic':
                utils.move.toElectronic();
                break;
            case 'moveambient':
                utils.move.toAmbient();
                break;
            case 'adddupe':
                utils.add.toDupes();
                break;
            case 'moveshuffle':
                utils.move.toShuffle();
                break;
            case 'movesahara':
                utils.move.toSahara();
                break;
...
utils.connectServer.setGood = function () {
    $("#status-icon").removeClass("label-danger").addClass("label-success").text('connected');

}

utils.connectServer.setBad = function () {
    $("#status-icon").removeClass("label-success").addClass("label-danger").text('disconnected');
}
...
var upi = function () {
    var spUri = 'spotify:user:jerblack:playlist:0g4DxWiFYVCHjhhQA9tU2W';
    var plName = 'Electronic/Dance 2';
    var compare = {
        name: plName,
        uri: spUri
    }
...
utils.migrate.server = function (plURI, iterations) {
    ...
    models.Playlist.fromURI(plURI, function (p) {
        ...
        local_uris = [];
        p.tracks.forEach(function (t) {
            if (count < iterations && t.uri.search('spotify:local:') != -1) {
                tUris.push(t.uri);
            }
            count++;
        })
...
utils.move.toPlaylist = function (cat) {
    ...
    if (!moveToSelf) {
        var entryType = "Move Track";
        var entryText = [];
        models.Playlist.fromURI(tarPl.uri, function (p) {
            entryText.push("Moving \"" + player.track.name + "\" to " + cats[cat].category);
            entryText.push("Added to " + tarPl.name);
        ...
        cats.forEach(function (c) {
            if (c.category != cats[cat].category) {
                ...
                c.playlists.forEach(function (pl) {
                    models.Playlist.fromURI(pl.uri, function (p) {
                        if (p.indexOf(t.uri) != -1) {
                            entryText.push("Removed from " + p.name);
                            p.remove(t.uri);
                        }
                    })
                })
        ...
        log(entryType, entryText);
    }
}

utils.move.toAmbient = function () {
    utils.move.toPlaylist(0);
};
utils.move.toElectronic = function () {
    utils.move.toPlaylist(1);
};
utils.move.toSahara = function () {
    utils.move.toPlaylist(2);
};
utils.move.toShuffle = function () {
    utils.move.toPlaylist(3);
};


utils.add.toPlaylist = function (pl) {
    var playlists = [
...
utils.add.toDupes = function () {
    utils.add.toPlaylist(0);
}
...
utils.count = function () {
    function addLine(text, right) {
        if (!right) {
            if (text != "-") {
                $("#count").append('<li><a href="#" >' + text + '</a></li>');
            } else {
                $("#count").append('<li role="separator" class="divider"></li>');
            }
        } else {
            $("#count").append('<li><a href="#" class="text-right">' + text + '</a></li>');
        }
    }

    $("#count").find("li:gt(0)").remove();

    var entryType = "Track Count";
    var entryText = [];
    ...
    cats.forEach(function (cat) {
        addLine('-');
        addLine("<strong>" + cat.category + "</strong>");
        var tcount = 0;
        cat.playlists.forEach(function (playlist) {
            models.Playlist.fromURI(playlist.uri, function (p) {
                count += p.length;
                tcount += p.length;
                addLine(p.name + ": " + p.length, true);
                p.tracks.forEach(function (t) {
                    if (t.local) {
                        locals++;
                    }
        ...
        addLine("<em>total in category: </em><strong>" + tcount + "</strong>", true);

        entryText.push(tcount + " in " + cat.category);
    })
    addLine('-');
    addLine("local tracks: <strong>" + locals + "</strong>", true);
    addLine("TOTAL tracks: <strong>" + count + "</strong>", true);
    entryText.push(locals + " tracks are local");
    entryText.push(count + " total tracks");
    log(entryType, entryText);
}
...
remove.artist.fromURI = function (uri) {
    var entryType = "Removing Artist";
    var entryText = [];
    if (uri.search('spotify:artist:') != -1) {
        models.Artist.fromURI(uri, function (a) {
            entryText.push("Removing all tracks from artist");
            entryText.push("a.name.decodeForText()");
            var count = 0;
            var pl = fpls.concat(spls);
            pl.forEach(function (p) {
                entryText.push("Now searching playlist '" + p.name.decodeForText() + "'");
                p.tracks.forEach(function (t) {
                    if (t.artists[0].uri == uri) {
                        p.remove(t.uri);
                        count++;
                        models.Track.fromURI(t.uri, function (tr) {
                            entryText.push("Removed track '" + tr.toString().decodeForText() + "'");
                        })
                    }
                })
            })
        ...
        $.getJSON("http://" + serverIP + "/cmd/gettracksforlocal/" + uri, function (data) {
            tracks = [];
            entryText.push("Removing all tracks for artist '" + data.name + '"');
            ...
            if (data.albums != undefined) {
                ...
                var pl = fpls.concat(spls);
                pl.forEach(function (p) {
                    entryText.push('', "Searching playlist '" + p.name + "'");
                    tracks.forEach(function (t) {
                        if (p.indexOf(t) != -1) {
                            p.remove(t);
                            models.Track.fromURI(t, function (tr) {
                                entryText.push('', "Removed '" + tr.toString() + "'");
                            })
                ...
                entryText.push('Artist for local track was not found on Spotify');
            }
        })
    } else {
        entryText.push('Invalid URI passed to remove.artist.fromURI: ' + uri);
    }
    log(entryType, entryText);
}
...
remove.track.fromURI = function (trackURI) {
    ...
    models.Track.fromURI(trackURI, function (t) {
        if (trackURI.search('spotify:local:') != -1) {
            ...
            entryText.push('on local track');
            entryText.push(trackURI);
        } else {
            npData = {
                "spURL": trackURI,
                "name": t.name.decodeForText(),
            ...
            entryText.push('Song: ' + t.name.decodeForText());
            entryText.push('Artist: ' + t.artists[0].name.decodeForText());
            entryText.push('Album: ' + t.album.name.decodeForText());
        }
        t.starred = false;
